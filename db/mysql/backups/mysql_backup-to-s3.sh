#!/bin/bash
###   Transport mysql db backups to s3 bucket: s3://$BUCKET_NAME/
...
bucket="$BUCKET_NAME"
backups_dir="$BACKUP_DIRECTORY_FULL_PATH"
today=$(date +%Y-%m-%d)
host=$(hostname -s)
...
#get list per matched string(db name) in filename(keyname) and delete these keys

#Clean Daily backups
echo "Cleaning daily backups older than $daily_keep_last num of files"
for key in $(aws s3api list-objects-v2 --bucket $bucket --query "reverse(sort_by(Contents[?contains(Key, '${db_name}') && contains(Key, 'sql.gz.enc') && starts_with(Key, 'name') == \`false\`], &LastModified))[${daily_keep_last}:].Key" --prefix "daily" --output text); do
	aws s3 rm s3://${bucket}/${key}
done

#Clean Weekly backups
echo "Cleaning weekly backups older than $weekly_keep_last num of files"
aws s3api list-objects-v2 --bucket $bucket --query "reverse(sort_by(Contents[?contains(Key, '${db_name}') && contains(Key, 'sql.gz.enc') && starts_with(Key, 'name') == \`false\`], &LastModified))[${weekly_keep_last}:].Key" --prefix "weekly" --output text|
while read -r key; do
        aws s3 rm s3://${bucket}/${key}
done
...
aws s3api list-objects-v2 --bucket $bucket --query "reverse(sort_by(Contents[?contains(Key, '${db_name}') && contains(Key, 'sql.gz.enc') && starts_with(Key, 'name') == \`false\`], &LastModified))[${monthly_keep_last}:].Key" --prefix "monthly" --output text|
while read -r key; do
        aws s3 rm s3://${bucket}/${key}
done
